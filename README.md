...
The inverse of adaptels and SICLE. Rather than partition the image, you supply
points and each grows into the region that looks like the pixel it sits on;
everything unseeded stays `-1`. Built to delineate standing dead trees from a
hand-digitised point layer — the operator supplies the objects, the algorithm
...
that point's attributes. It runs on the same frozen IFT kernel as SICLE, which
keeps it bit-identical to the R twin in
[rgeoadaptels](https://github.com/igorpawelec/rgeoadaptels). See
[`docs/grow_seeds_guide.md`](docs/grow_seeds_guide.md).
